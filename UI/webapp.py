...
def run_driver_management():
    if request.method == 'POST':
        ...
        command_driver_actions = "source " + ros_path + "install/setup.bash && source /opt/intel/openvino/bin/setupvars.sh && cd " + ros_path + \
            "src/ets_ros2/aws-crt-cpp/samples/OpenVino-Driver-Behaviour/ActionRecognition && python3 action_recognition.py -m_en models/FP32/driver-action-recognition-adas-0002-encoder.xml -m_de models/FP32/driver-action-recognition-adas-0002-decoder.xml -lb driver_actions.txt -d CPU"
        ...
        command_driver_behaviour = "source /opt/ros/crystal/setup.bash && source /opt/intel/openvino/bin/setupvars.sh && source " + ros_path + "src/ets_ros2/aws-crt-cpp/samples/OpenVino-Driver-Behaviour/" + dmanagement_repository + "/scripts/setupenv.sh && cd " + ros_path + "build/aws-crt-cpp/samples/OpenVino-Driver-Behaviour/" + dmanagement_repository + "/intel64/Release && ./driver_behavior -d " + \
            json['target'] + " -m_hp $hp32 -d_hp CPU -dlib_lm -d_recognition -fg " + ros_path + "src/ets_ros2/aws-crt-cpp/samples/OpenVino-Driver-Behaviour/" + \
